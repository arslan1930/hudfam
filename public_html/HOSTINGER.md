...
5. **Email campaigns** (separate): Admin fills per-country sheets (URL + email) → export Ready CSV → send → mark Emailed. Team marks Replied/Dealing to cut from future sends
6. Catalog Super search still shows site metrics only (no client/project secrets)
