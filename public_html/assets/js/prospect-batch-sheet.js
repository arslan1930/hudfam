/**
 * Admin · Site adding history day — editable domain list with undo/redo + autosave.
 */
(function () {
  'use strict';

  var shell = document.getElementById('history_sites_shell');
  var ta = document.getElementById('history_sites_text');
  if (!shell || !ta) return;

  var postUrl = shell.getAttribute('data-post-url') || window.location.href;
  var copyBtn = document.getElementById('history_copy_all');
  var undoBtn = document.getElementById('history_undo_btn');
  var redoBtn = document.getElementById('history_redo_btn');
  var statusEl = document.getElementById('history_list_status');
  ...
  function runAutosave() {
    ...
    fetch(postUrl, {
      ...
      .then(function (data) {
        ...
        updateCounts(n);
        setAutosaveLabel('Saved');
        var bits = ['Autosaved · ' + n + ' site' + (n === 1 ? '' : 's')];
        if (data.removed) bits.push('removed ' + data.removed + ' from history');
        if (data.inserted) bits.push('added ' + data.inserted);
        if (data.db_removed) bits.push('removed ' + data.db_removed + ' from Our database');
        if (data.removed || data.inserted || data.db_removed) {
          setStatus(bits.join(' · '));
        }
      })
      .catch(function (err) {
  ...
  if (copyBtn) {
    copyBtn.addEventListener('click', function () {
      ...
      copyText(text)
        .then(function () {
          setStatus('Copied ' + lines.length + ' site' + (lines.length === 1 ? '' : 's') + '.');
        })
        .catch(function () {
