...
function guide_invoices(): string
{
    return render_page_purpose(
        ...
        [
            'Generate invoice: tick unpaid LIVE rows pushed from Order management (opening Generate from Invoices starts with none ticked). Tick one bill-as only — mixed emails/names cannot share a bill. New invoice gets the next number. Add to existing puts more sites on an unpaid bill with the same bill-as (Draft or already generated) — same invoice number. Paid invoices stay locked. Group same amount is off unless you turn it on.',
