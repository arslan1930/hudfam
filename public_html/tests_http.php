...
if ($rUnpaidList['status'] === 200
    ...
    fail('admin invoices unpaid working view status=' . ($rUnpaidList['status'] ?? '?'));
}
if ($r['status'] === 200
    ...
    fail('admin invoices all missing');
}
...
$openAt = strpos($finalHub, 'id="swe-open-country"');
if ($r['status'] === 200
    && $openAt !== false
    ...
    && !str_contains($finalHub, 'Open a country to paste or import —')
    && ($tableAt === false || $tableAt < $openAt)
...
if ($r['status'] === 200 && str_contains($r['body'], 'Campaign drafts')
    ...
    && str_contains($r['body'], 'data-camp-draft-suggest')
    && str_contains($r['body'], 'data-camp-draft-abc')
    && str_contains($r['body'], 'data-camp-draft-expand')
    && str_contains($r['body'], 'Italy, niche, discount, homepage')
...
if ($r['status'] === 200
    && str_contains($r['body'], 'value="sample"')
    && str_contains($r['body'], 'Sample · they asked for an example · A')
    && str_contains($r['body'], 'data-camp-draft-suggest')
    && str_contains($r['body'], 'Payment has been sent · A')) {
    pass('comms Campaign drafts search keeps the full library in the page');
} else {
    fail('comms Campaign drafts search did not render live-filter markup');
}
$r = req('GET', $base . '/index.php?page=team_email_campaigns_drafts&q=' . rawurlencode('homepage'));
if ($r['status'] === 200
    && str_contains($r['body'], 'value="homepage"')
...
if ($r['status'] === 200
    && str_contains($r['body'], 'value="Italy"')
...
if ($r['status'] === 200
    && str_contains($r['body'], 'value="Germany"')
...
$r = req('GET', $base . '/index.php?page=team_email_campaigns_drafts&q=' . rawurlencode('they asked for niche'));
if ($r['status'] === 200
    && str_contains($r['body'], 'Topics · they asked for niche / category · A')
    && str_contains($r['body'], 'Topics · they asked for niche / category · B')
    && str_contains($r['body'], 'Topics · they asked for niche / category · C')
    && str_contains($r['body'], 'not generated by AI')
    && str_contains($r['body'], 'data-camp-draft-abc')) {
...
if ($r['status'] === 200
    ...
    && str_contains($r['body'], 'Sample · Italy · A')
    && str_contains($r['body'], 'Sample · they asked for an example · A')
    && str_contains($r['body'], 'data-camp-draft-chip="folder"')
    && str_contains($r['body'], 'data-camp-draft-folder')
    && str_contains($r['body'], 'First outreach · requesting · A')) {
    pass('comms Campaign drafts Samples folder chip still lists Samples cards');
} else {
