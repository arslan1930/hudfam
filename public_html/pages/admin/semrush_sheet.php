<?php
/**
 * Admin · Semrush Research · one country sheet (keeps Admin chrome).
 */
$user = require_admin();
...
if (!$canon) {
    ...
    redirect(semrush_hub_url(true));
}
$country = $canon['name'];
$isAdmin = true;
$base = semrush_sheet_url($country, true);
...
render_header('Semrush · ' . $country, 'admin');
render_breadcrumbs([
...
<div class="topbar">
  <div>
    ...
    <p class="muted">
      ...
      Admin sheet · edit, copy, cut, undo/redo · comments below
    </p>
  </div>
  <div class="actions">
...
<div class="card">
  ...
  <form method="post" action="<?= h($base) ?>" style="margin-top:0.85rem"
        <?= confirm_attrs(
            'Clear all site names and comments for ' . $country . '?'
    ...
    <?php foreach ($comments as $c):
      ...
      <li>
        ...
        <form method="post" action="<?= h($base) ?>#semrush-comments" class="semrush-comment-delete"
              <?= confirm_attrs('Delete this comment?', ['title' => 'Delete comment?', 'confirm_label' => 'Delete', 'danger' => true]) ?>>
          <?= csrf_field() ?>
